...
export async function setTournamentRegimeAdminFlag(event, context, callback) {
  context.callbackWaitsForEmptyEventLoop = false;

  const cognitoSub = event.cognitoPoolClaims.sub;

  const { tournamentRegimeId, adminFlag } = event.body;
  ...
  try {
    ...
    const result = await request.execute('dbo.up_AdminUpdateTournamentRegimeTestOnlyFlag');
